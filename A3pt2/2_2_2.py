...
def defineGraph(trainData, clusterCentroids, covarianceMatrix, priors):
    ...
    const_term = priors + tf.log(tf.cast(((2*pi*covarianceMatrix)**(D/-2.)),tf.float32))
    ...
    return loss, result, clusterAssignments,const_term,powerof
...
if __name__ == '__main__':
    ...
    trainLoss, result, trainAssignments,const_term,exp_term = defineGraph(trainData, clusterCentroids, covMatrix,priors)
    if FLAGS.validate == True:
        validLoss, result, validAssignments, const_term,exp_term = defineGraph(validData, clusterCentroids, covMatrix,priors)
    ...
    with sess.as_default():
        ...
        while epoch_num < FLAGS.num_epochs:
            ...
            a,b,cov, pri, means,const,d= sess.run([trainLoss,clusterCentroids, covMatrix, priors, clusterCentroids,const_term,exp_term])
            print "train loss ", a
            loss.append(a)
        ...
        #Done training
        finalTrainData, finalTrainAssignments, finalCentroids, finalCov,finalprior = sess.run([trainData, trainAssignments, clusterCentroids,covMatrix,psi])
        print "final result: ", finalTrainAssignments
        if FLAGS.validate == True:
            finalValidData, finalValidAssignments = sess.run([validData, validAssignments])


    #plot k=3 and the loss and the variables
    # print "cc: ", finalCentroids
    # print "priors ", finalCov
    # print "cov ", finalprior
    # plt.plot(x_axis,loss, label='LOSS', color='b')
